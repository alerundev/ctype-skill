...
| 종류 | 키 예시 | 등록 방법 |
|---|---|---|
| 외부 API 인증 정보 | `OPENAI_API_KEY`, `STRIPE_SECRET_KEY` | `ctype stage secret` |
| DB 연결 정보 | `DATABASE_URL`, `DB_PASSWORD` | `ctype stage secret` |
...
DB preset 의 디폴트 설정 (`ctype preset list -o json` 결과 활용) 을 그대로 사용하고, `rootpassword` 를 plain 문자열로 박습니다. 같은 값을 stage 시크릿에도 등록합니다.
...
```bash
# 시크릿 등록 (앱이 사용할 형태로)
ctype stage secret DB_PASSWORD "<같은 패스워드>"
ctype stage secret DATABASE_URL "postgresql://root:<패스워드>@postgres:5432/appdb"
...
DB 가 `Running` 인 것을 확인한 뒤 다음 단계로 진행합니다.

### 5.2 백엔드 시크릿/env 등록

3단계에서 정리한 키 목록을 등록합니다.

```bash
# 시크릿 (이름 패턴이 PASSWORD/SECRET/TOKEN/KEY/PWD/PRIVATE/AUTH 또는 자격증명 포함 URL)
ctype stage secret OPENAI_API_KEY "<값>"
ctype stage secret STRIPE_SECRET_KEY "<값>"

# 평문 (단순 플래그)
ctype stage variable NODE_ENV production
ctype stage variable LOG_LEVEL info
```
...
| 원인 | 처리 |
|---|---|
| 브랜치 불일치 (`Couldn't find remote ref`, `branch not found` 등) | `python scripts/find_repo.py --branches <git-url>` 로 실제 브랜치 목록 조회 → `context.git.ref` 수정 → 재배포 |
| 포트 불일치 (`EADDRINUSE`, connection refused, healthcheck 실패 등) | 실행 로그에서 실제 listen 포트 확인 → `options.ports` 수정 → 재배포 |
| DB 연결 실패 (`connection refused`, `password authentication failed`) | 백엔드 시크릿의 호스트/포트/패스워드가 5.1 의 값과 일치하는지 확인 → `ctype stage secret` 재등록 → 재배포 |
...
DB preset 의 `rootpassword` 는 plain 문자열만 동작합니다. 시크릿 참조 객체를 넣으면 배포 후 `[ServiceError] secret value must be a string` 으로 `stopped` 상태가 됩니다. 같은 값을 `ctype stage secret` 으로 따로 저장하고 앱은 그 시크릿을 참조하는 패턴을 사용합니다.
...
- [`reference/api.md`](reference/api.md) — 이 스킬이 호출하는 3개 API: 빌드 로그 / 실행 로그 / GitHub repo 조회
