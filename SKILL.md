...
스킬 진입 시 맩닱적으로 한 번. 이미 되어 있으면 건너뛰고, 안 돼 있을 때만 실행.
...
배포에 필요한 `context.git.url` 은 세 경로 중 하나로 확보합니다.

1. **상위 에이전트가 코딩·push 까지 한 경우** (메인 경로): 자기가 push 한 URL 을 그대로 사용.
2. **사용자가 URL 을 직접 준 경우**: 그대로 사용.
3. **사용자가 이름/키워드만 준 경우** ("내 주소 축약기 배포"): 아래 helper 스크립트로 매칭.
...
helper 가 사용하는 API endpoint 와 동작 세부: [`reference/api.md`](reference/api.md) 의 "GitHub repo 조회".
...
```bash
ctype presets                     # 사용 가능한 preset 목록 (표)
ctype preset list -o json         # 각 preset 의 디폴트 설정 (app/ports/start/env)
```

### 상위 에이전트가 코드를 소유한 경우 (메인 경로)

repo 내용 (`package.json` / `requirements.txt` / `pom.xml` / `Dockerfile` / `go.mod` / ...) 을 보고 `ctype presets` 의 example URL 과 매칭. 해당 preset 의 `config[].options` 를 읽어 그 구조를 따른 yaml 을 작성.

### 상위 에이전트가 사용자 repo URL 만 받은 경우

**사용자에게 물을 것 = preset 이름 하나**:

> "어떤 preset 으로 배포할까요? (예: `node`, `python-flask`, `java-springboot`)"

**포트는 안 물음**. preset 마다 글로벌 표준 포트가 `ctype preset list -o json` 의 `config.options.ports` 에 박혀 있어 그걸 그대로 쥌 (Node 3000, Flask 5000, Django/FastAPI 8000, Spring/Go 8080 등). 맞으면 한 사이클에 배포 성공, 안 맞으면 6단계 실패 대응 흐름 (로그 → 수정) 로 처리.

DB 가 필요한데 종류 미명시이고 사용자에게 묻기 곤란한 컨텍스트면 `postgresql` 을 디폴트로 사용하고 완료 보고에 명시합니다.

### preset 디폴트의 희말 수준 (계측 사실)

`cloudtype-examples/<preset>.git` 처럼 **표준 구조의 repo 는 yaml 에 `name` + `app` + `context.git` + `options.ports` 만 박아도 배포 성공함**. `start` 명령은 `package.json` 의 `scripts.start` 같은 표준 진입점을 Cloudtype 이 자동으로 호출. 코드가 비표준 구조면 (예: Flask 인데 `app.py` 가 아닌 `main.py`) 그때만 `start` override 추가.
...
1. 2단계에서 결정한 preset 을 `ctype preset list -o json` 결과에서 찾아 `config[0].options` 를 확인. 거기 박힌 표준 값 (`ports`, `start`, `env`) 을 yaml 에 그대로 사용.
2. 용도만 다른 항목 (deployment name, git url, branch) 을 추가.
3. 사용자 코드가 비표준 구조일 경우 (예: Flask 인데 `app.py` 가 아닌 `main.py`) 그때만 `options.start` 를 명시적으로 override.

**최소 형태** (Node 예시, 표준 `npm start` 구조):
...
여기서 `app: node@16` 와 `ports: "3000"` 은 `ctype preset list -o json` 의 `node` preset 디폴트 그대로. `start` 은 생략 — Cloudtype 이 `package.json` 의 `scripts.start` 자동 호출.

필드 가이드 (preset 별 옵션, 시크릿 문법, 멀티 서비스): [`reference/yaml.md`](reference/yaml.md).

**작성 원칙**:
- 사용자가 명시한 항목만 yaml 에 박습니다. 나머지는 서버 디폴트 + preset 디폴트.
- `context.git.url` 은 사용자 또는 상위 에이전트가 제공하거나 1단계의 `find_repo.py` 로 확정하며, 끝내 알 수 없으면 사용자에게 묻습니다.
